# E060_ToddHughes_TheHauntedShrine

_Source: `E060_ToddHughes_TheHauntedShrine.pdf`_
_Processed: 2026-06-28T05:04:17.754Z_
_Model: `gemini-3.1-flash-lite` (medium reasoning)_

## Transcription

### Page Description
The document is a single-page tabletop roleplaying game (TTRPG) dungeon module titled "The Haunted Shrine" by Todd Hughes. It is laid out in a clean, utilitarian style. At the top left, there are two black-and-white grid maps labeled "Level 1" and "Level 2." To the right of the maps is a boxed-off section titled "Background." Beneath the maps and background text, there is a numbered list ("Key") that details the contents of ten specific rooms or areas, along with a legend explaining the map symbols (1 square = 10 feet, 's' = secret door, '++' = stairs).

---

### Transcribed Text

**The Haunted Shrine**
http://creativecommons.org/licenses/by-sa/3.0/

**Legend:**
1 square = 10 feet
s = secret door
++ = stairs

**Background**
The shrine in a local town has been abandoned for months because of the rumors of a haunting. After the long time priest was murdered in the shrine people began seeing his ghost in the halls. Lately people have been disappearing, mostly travelers passing through, and townsfolk blame the ghosts in the shrine.

**

title: The Haunted Shrine
summary: A shrine abandoned after the murder of its priest becomes haunted and is blamed for the disappearance of travelers. The dungeon features two levels with secret doors and stairs, but lacks detailed encounter descriptions.
rooms: 10
resolutions: Exploration
concept_originality: 2
mechanics_originality: 1
interesting_details: 2
map_quality: 3
humor: 2.5
content_rating: PG
rated_at: 2026-06-28T06:19:50.082Z
model: deepseek/deepseek-v4-flash
